...
const ciclosService = new crud_service_1.CrudService('ciclos_escolares', 'id_ciclo', true);
// CRUD base
const baseCrudRoutes = (0, crud_routes_1.buildCrudRoutes)(ciclosService, 'id');
const ciclosRoutes = async (fastify) => {
    ...
    fastify.post('/auto', async (request, reply) => {
        ...
        `, [todayStr]);
            ...
            const periodo = month <= 6 ? 'ENE-JUN' : 'AGO-DIC';
            let nombre;
            let fecha_inicio;
            let fecha_fin;
            if (periodo === 'ENE-JUN') {
